...
def initialize(*args)
  super
  new_resource.service_type( (new_resource.service_type or node['undef-dashing']['service_type']) )

end

def load_current_resource
    @current_resource = Chef::Resource::UndefDashingDashboard.new(@new_resource.name)
    @current_resource.name(@new_resource.name)
    @current_resource.path(@new_resource.path)
    @current_resource.port(@new_resource.port)
    @current_resource.service_type(node['init_package'])
end
...
def setup_dashboard(current_resource, new_resource)
    ...
    begin
        ...
        file "#{cached_new_resource.path}/.ruby-version" do
            content node['undef-dashing']['ruby_version']
            owner node['undef-dashing']['user']
            group node['undef-dashing']['group']
            mode "0544"
        end
        ...
        script "#{get_dashboard_name()} - chown dashboard" do
            ...
            cwd cached_new_resource.path
            code <<-EOH
            chown -R "#{node['undef-dashing']['user']}":"#{node['undef-dashing']['group']}" .
            EOH
        end
...
def create_service(resource, actions)
    ...
    when 'systemd'
        ...
        raise "undef-dashing: Unknown service_type '#{resource.service_type}'"
    end
end
...
def create_service_files(current_resource, new_resource)
    ...
    template_variables = ({
        ...
        'ruby_version' => node['undef-dashing']['ruby_version']
    })
    ...
    when 'systemd'
        template "/etc/systemd/system/#{service_name}.service" do
            cookbook 'undef-dashing'
            source 'systemd-dashboard.service.erb'
            mode 0755
            owner "root"
            group "root"
            variables template_variables
...
def get_service_script_name(resource=nil)
    ...
    when 'system'
        answer = "/etc/systemd/system/#{get_service_name(resource)}.service"
    end
    return answer
end
...
def get_service_name(resource=nil)
    if resource == nil then resource = @new_resource end
    return "undef-dashing-#{get_dashboard_name(resource)}"
end
